...
impl BpeTrainer {
    ...
    pub fn do_train(
        ...
        loop {
			...
			// Only do this once, before the loop, not inside it!
			let words: Vec<RefCell<Word>> = words.into_iter().map(RefCell::new).collect();
			...
			loop {
				...
				// ✅ SAFE merge using RefCell
				let changes = top
					.pos
					.iter()
					.flat_map(|i| {
						let mut word = words[*i].borrow_mut();
